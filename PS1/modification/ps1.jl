...
function simulation(param1::AbstractVector, fixed_param::parameters, market::AbstractVector, firm_char::AbstractVector, potential::AbstractVector, rng::MersenneTwister)
    """
    Input:
    1. param1::AbstractVector : parameters of interest [μ, σ, δ] 
    2. fixed_param::parameters : other parameters [α, β, M]
    ...
    for i in eachindex(potential)
        # unobservable part of firm generating
        u_firm = rand(rng, Normal(param1[1], param1[2]), potential[i])
        u_m = push!(u_m, u_firm)
    end
    for m in eachindex(potential) # Market m case
      x = market[m]
      ...
      ## each firm's profit 
      Π_m = zeros(eltype(Float64),entr_num)
      Π_m = x * fixed_param.β .- Z_m * fixed_param.α - u_m[m]
      # order firms by profitability
      sort!(Π_m, rev = true)
...
function simu_estimator(param1::AbstractVector, fixed_param::parameters, market::AbstractVector, firm_char::AbstractVector, eq_firm::AbstractVector , potential::AbstractVector ,S::Int64)
    ...
    for s in 1:S 
        simu_firms[:,s] = simulation(param1, fixed_param, market, firm_char, potential, seed)
    end
...
################################################################################################
######################## Moment inequality estimator ###########################################
################################################################################################


"""
X::AbstractVector: market observables
Z::AbstractVector: heterogeneous firm observables

"""

"""
Steps:
1) draw psedo random numbers
2) get profits for each firm 
3) 
"""
# draw random numbers 
"""
ϵ_firm = zeros(eltype(Float64),param.M, S* entrant[m])




"""
function makeindex(firm_number::Int64)
    index = zeros(eltype(Int64),2^firm_number, firm_number)
    for i in 1: firm_number
        ii = 1
        cc = 1
        c = Int64(2^firm_number / (2^i))
        while ii <= 2^firm_number
            if cc <= c
                index[ii,i] = 1
                cc += 1
                ii += 1
